...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count 	= 0; 					// I assume that this 0 value is correct since this value is not defined on the website
	...
	superblock.s_blocks_per_group 	= EXT2_S_IFCHR; 		// 8192
	superblock.s_frags_per_group 	= EXT2_S_IFCHR; 		// 8192
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; // fixed to 2 used directories for this project: root and lost+found
...
void write_block_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value[BLOCK_SIZE]; // free-range: 24-1023
...
void write_inode_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value[BLOCK_SIZE]; // free range 14-128
	map_value[0] = 0b11111111; // 1-8
	map_value[1] = 0b00011111; // 9-16
	for (u32 i = 2; i < 16; i++) { // 17-128 
		map_value[i] = 0b00000000;
	}
	for (u32 i = 16; i < BLOCK_SIZE; i++) { //129+
		map_value[i] = 0b11111111;
	}
...
void write_inode_table(int fd) {
	...
	root_inode.i_mode = EXT2_S_IFDIR
	                              ...
	                              | EXT2_S_IXGRP
	                              | EXT2_S_IROTH
	                              | EXT2_S_IXOTH; // update permissions based on spec
	root_inode.i_uid = 0;
	...
	// 2. lost+found directory -> completed above. 
	// 3. hello-world file
	...
	hello_world_inode.i_mode = EXT2_S_IFREG
	                              ...
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH; // update permissions based on spec
...
void write_hello_world_file_block(int fd)
{
	...
	//use write() system call to write contents of file into predetermined location
	//contents: "Hello world\n"
	ssize_t bytes_remaining = BLOCK_SIZE;

	char* content = "Hello world\n";
	if (write(fd, content, strlen(content)) != strlen(content)) { // should be 12 written bytes
		errno_exit("write");
	}
